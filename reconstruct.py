...
def gen_pt_cloud(i, prev_sensor, K_matrices, image1, image2, poses, choice):
    ...
    if choice == 'features':
        ...
        src_pts, dst_pts, filtered_kp1, filtered_kp2 = filter_keypoints(mask, src_pts, dst_pts, filtered_kp1, filtered_kp2)
        ...
        img1_pts, img2_pts, img_colours = get_colours(img1, K_matrices, norm_pts1, norm_pts2, homog_3D)

        print "    Initializing feature tracks..."
        pt_cloud_indexed = attach_indices(i, pts_3D, filtered_kp1, filtered_kp2)

        return sensor_i, K_matrices, kp2, des2, filtered_kp2, homog_3D, pts_3D, img_colours, pt_cloud_indexed
...
def find_new_pts_feat(i, prev_sensor, K_matrices, image1, image2, prev_kp, prev_des, prev_filter, poses, pt_cloud_indexed):
    print "    Loading images..."
    img1, img2 = load_images(image1, image2)
    img1_gray, img2_gray = gray_images(img1, img2)
    print "    Building camera calibration matrices..."
    ...
    return sensor_i, K_matrices, new_kp, new_des, filtered_kp2, poses, homog_3D, pts_3D, img_colours, pt_cloud_indexed
...
def start(choice, images, file_path=None):
    ...
    for i in range(len(images)-1):
        ...
        if choice == 'features':
            if i == 0:
                # first 2 images
                prev_sensor, K_matrices, prev_kp, prev_des, prev_filter, homog_3D, pts_3D, img_colours, pt_cloud_indexed = gen_pt_cloud(i, prev_sensor, K_matrices, images[i], images[i+1], poses, choice)
                pt_cloud = np.array(pts_3D)
                colours = np.array(img_colours)
            elif i >= 1:
                prev_sensor, K_matrices, prev_kp, prev_des, prev_filter, poses, homog_3D, pts_3D, img_colours, pt_cloud_indexed = find_new_pts_feat(i, prev_sensor, K_matrices, images[i], images[i+1], prev_kp, prev_des, prev_filter, poses, pt_cloud_indexed)
